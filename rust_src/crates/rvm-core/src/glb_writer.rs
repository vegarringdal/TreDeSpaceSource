...
/// One merged, single-colour node captured straight out of the merged build —
/// the same buffers the GLB would have carried, without the glTF wrapper.
///
/// `positions` are glTF-space (**Y-up**, xyz triples): the merged build rotates
/// Z-up → Y-up while flattening, and the cooker bridge rotates back with the
/// same `[x, -z, y]` it applies when reading these buffers out of a GLB. Keeping
/// the round trip means the direct path and the GLB path cook identical bytes.
...
/// A whole merged export in memory — what `build` would have serialised.
pub struct MergedData {
    pub nodes: Vec<MergedNodeData>,
    /// `(id, name, parent_id)`; `parent_id == 0` marks a root.
    pub hierarchy: Vec<(u32, String, u32)>,
    pub bbox: BBox3,
}
...
impl GlbWriter {
    ...
    pub fn build(
        ...
        let (glb, bbox, _) = Self::build_inner(
            nodes,
            colors,
            remove_empty,
            cleanup_positions,
    ...
    /// The merged build WITHOUT the glTF wrapper: same geometry, dedup, meshopt
    /// and draw ranges, handed back in memory for a direct cook. Skips the
    /// buffer/accessor/JSON work entirely — nothing is serialised.
    pub fn build_merged(
        nodes: &mut HashMap<u32, MetaNode>,
        colors: &[u32],
        remove_empty: bool,
        ...
        let (_, bbox, merged) = Self::build_inner(
            nodes,
            colors,
            remove_empty,
            cleanup_positions,
    ...
    fn build_inner(
        ...
        meshopt_target_error: f32,
        want_glb: bool,
    ) -> (Vec<u8>, BBox3, Option<MergedData>) {
        ...
        for &color in colors {
            ...
            let (final_indices, final_positions, vert_count) = if cleanup_positions {
                ...
                for id in &sorted_ids {
                    ...
                    // trunc each coordinate to `cleanup_precision` decimal places and
                    ...
                    for i in local_start..local_end {
                        ...
                        let key = (
                            (x * scale as f32).trunc() as i64,
                            (y * scale as f32).trunc() as i64,
                            (z * scale as f32).trunc() as i64,
                        );
            ...
            // ── direct (GLB-free) path: capture the merged node and skip
            //    every buffer / accessor / JSON step below ────────────────
            if !want_glb {
                let mut ranges: Vec<(u32, u32, u32)> = Vec::new();
                for id in &sorted_ids {
                    let node = nodes.get(id).unwrap();
                    if node.color_with_alpha != color
                        || node.primitives.is_empty()
                        || node.count == 0
                    {
                        continue;
                    }
                    ranges.push((node.id, node.start, node.count));
                }
        ...
        // ── hierarchy: `(id, name, parent)` for the direct path, the
        //    `id_hierarchy` extra for the GLB ──────────────────────────────
        if !want_glb {
            let hierarchy = sorted_ids
                .iter()
                .map(|id| {
                    let node = nodes.get(id).unwrap();
                    (node.id, node.name.clone(), node.parent_id)
                })
                .collect();
            let merged = MergedData {
                nodes: merged_nodes,
                hierarchy,
                bbox: global_bbox.clone(),
            };
            return (Vec::new(), global_bbox, Some(merged));
        }
        ...
        // No meshes ⇒ no triangles survived tessellation/cleanup. Return empty bytes so
        // the caller skips the file (and its status_file.json entry) instead of writing
        // a valid-but-empty GLB.
        if meshes.is_empty() {
            return (Vec::new(), BBox3::default(), None);
        }
    ...
    pub fn build_instanced(
        ...
        // No meshes ⇒ no triangles; skip the file (see `build`).
        if meshes.is_empty() {
            return (Vec::new(), BBox3::default());
        }
...
fn weld_and_simplify(positions: &[f32], indices: &[u32], c: &Cleanup) -> (Vec<f32>, Vec<u32>) {
    // Step 1: precision-keyed weld (trunc coords to `cleanup_precision` decimals).
