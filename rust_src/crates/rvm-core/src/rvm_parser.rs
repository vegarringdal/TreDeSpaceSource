...
/// Cook hook: turns one merged export (in memory, no GLB) plus its output base
/// name into the bytes to write. Set it and merged mode emits `.tdp` instead of
/// `.glb` — see [`crate::convert::convert_cooked`].
pub type CookHook = Box<dyn Fn(crate::glb_writer::MergedData, &str) -> Result<Vec<u8>, String>>;
...
impl RvmParser {
    ...
    /// Emit cooked `.tdp` instead of `.glb` in merged mode: the merged export is
    /// handed to `hook` in memory, so no GLB is ever built or serialised.
    pub fn set_cook_hook(&mut self, hook: CookHook) {
        self.cook = Some(hook);
    }
            ...
            match cid {
                ...
                CNTB => {
                    ...
                    if self.level == (self.export_level + 1) as u32 {
                        ...
                        // Ancestor ids on the stack belong to containers outside this
                        // export (and clash with the restarted id counter). Zero them so
                        // the new root's parent_id is 0 → "*" in id_hierarchy.
                        for id in &mut self.parent_stack {
                            *id = 0;
                        }
                    }
    ...
    fn flush_root(&mut self, sink: &mut dyn OutputSink, progress: &mut dyn FnMut(&Progress)) {
        ...
        let (glb_bytes, bbox) = match self.mode {
            ...
            // Merged + cook hook: build the export in memory and cook it —
            // no GLB is assembled. Any other mode ignores the hook (only the
            // merged shape carries the draw ranges the cooker needs).
            OutputMode::Merged if self.cook.is_some() => {
                let merged = GlbWriter::build_merged(
                    &mut self.nodes,
                    &colors,
                    self.remove_empty,
                    self.remove_dups,
                ...
                let bbox = merged.bbox.clone();
                if merged.nodes.is_empty() {
                    (Vec::new(), bbox)
                } else {
                    let cook = self.cook.as_ref().unwrap();
                    match cook(merged, &sanitize_filename(&self.current_root_name)) {
                        Ok(bytes) => (bytes, bbox),
                        Err(e) => {
                            self.errors.push(format!("cook failed: {e}"));
                            (Vec::new(), bbox)
                        }
                    }
                }
        ...
        if !glb_bytes.is_empty() {
            ...
            let ext = if self.cook.is_some() && self.mode == OutputMode::Merged {
                "tdp"
            } else {
                "glb"
            };
