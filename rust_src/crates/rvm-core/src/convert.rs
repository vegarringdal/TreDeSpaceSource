...
/// each site's merged export in memory (positions, indices, draw ranges,
/// hierarchy) and returns the bytes to write, so no GLB is built or parsed.
///
/// The cooker lives outside this crate, hence the callback — see
/// `rvm_wasm::to_cooker_model` for the bridge the app uses.
pub fn convert_cooked(
    input: Box<dyn InputHandle>,
    sink: &mut dyn OutputSink,
    opts: &ConvertOptions,
    progress: &mut dyn FnMut(&Progress),
    cook: crate::rvm_parser::CookHook,
) -> Result<ConvertReport, String> {
    let mut parser = RvmParser::new(opts);
    parser.set_cook_hook(cook);
    parser
        .run(input, sink, progress)
        .map_err(|e| e.to_string())?;
    Ok(parser.into_report())
