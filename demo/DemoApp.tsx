...
import { APP_URL, IS_DIALOG, IS_POPUP } from './hostEnv';

function DemoShell() {
  const { attachIframe } = useDemo();

  // dialog mode (?dialog=1): this page is EMBEDDED IN the viewer (External app
  // panel) and drives window.parent; popup mode (?popup=1): OPENED by a demo
  // page and drives window.opener — neither has an iframe of its own; actions
  // fill the page, console docked at the bottom
  if (IS_DIALOG || IS_POPUP) {
    return (
 ...
 *  copy-paste SDK. Three host modes on the same page — default embeds the app
 *  as an iframe; ?dialog=1 is for when this page itself is HOSTED INSIDE the
 *  viewer (External app panel / dialog): no iframe, just actions + console,
 *  driving window.parent; ?popup=1 is a window another demo page OPENED and
 *  relays for (driving window.opener). Shipped with the build at <app>/demo/ . */
