...
- 1.1 **HIGH** — a chunk-upload session has no owner-death or inactivity handling; the import lock, the OPFS writable and the loading overlay stay stuck for the session.
...
- 3.1 **HIGH** — MD5 of every imported file runs synchronously on the main thread over the whole file.
...
- 3.2 **HIGH** — GLB export allocates vertex buckets sized by INDEX count.
...
- 3.3 **HIGH** — `.tdp` export re-cooks through the TS cooker with plain-array byte pools.
...
- 3.4 **HIGH** — merged-GLB import parses and cooks the whole GLB twice for the coarse variant.
...
- 6.1 **HIGH** — no per-client capability scoping: every allowed origin gets the full command table.
